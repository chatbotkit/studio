...
Auxiliary ports must be available; Studio fails startup instead of attaching to another application's listener. Container listeners come from the resolved platform `PORT` and `RELAY_PORT` environment and Garage's inline configuration, with no compiled-in port values. Garage's S3 port must equal the allocated storage port on both sides. Studio validates actual listener collisions, including the declared Garage admin port, before starting the VM. The bridge derives its targets from these resolved container settings, with the first target using the bare Unix socket and subsequent targets using a port suffix.

Readiness commands come from each service's Compose `healthcheck.test` (`CMD`, `CMD-SHELL`, or shell-string form). They run only inside the matching container with its inherited environment; no upstream command executes on the Mac. Required checks must be present and enabled. The adapter retains bounded native probe timeouts and startup retries.
