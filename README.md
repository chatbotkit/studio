...
Studio → Settings → Storage shows allocated cache/data sizes and previews obsolete-cache cleanup. Stack → Manage Storage opens that settings tab directly. Cleanup requires confirmation, stops the stack, rechecks the preview, and preserves persistent volumes, backups, and service disks before restarting. Missing last-known-good image metadata makes cleanup retain all image references. Size estimates may double-count shared APFS blocks. Free-space checks reserve 512 MiB of headroom and conservatively budget image pulls and disk creation; they cannot guarantee another application will not consume space mid-operation. Disk replacements are staged and promoted only after successful creation and synchronization.

Studio → Settings → Models configures the platform-wide credentials that make model providers available. Studio shows only whether a provider is configured; saved secrets are never read back into the macOS UI. Updates are sent through the running private container over standard input, atomically persisted as owner-only `/data/config.env` inside the existing `platform-data` disk, and applied by the platform’s own configuration watcher. OpenAI, OpenRouter, Vercel AI Gateway, Google AI, Amazon Bedrock, Cloudflare Workers AI, Perplexity, Mistral, Groq, and DeepSeek are supported. Removing a provider requires confirmation.
